  ...
  videos: [
    {
      ...
      metadata: {
        ...
        description: "A large and lovable rabbit deals with three bullying rodents in this classic open-source animated film by the Blender Foundation. Served as an HLS playlist folder from IPFS.",
        author: "Blender Foundation",
        entry: "master.m3u8",
        tags: ["animation", "hls", "open-source", "bunny"],
        thumbnail: "https://upload.wikimedia.org/wikipedia/commons/c/c5/Big_Buck_Bunny_Screen_Shot.jpg"
      }
    },
    {
      ...
      metadata: {
        ...
        description: "A short sample HLS video playlist containing several segment chunks. Ideal for verifying gateway connections and node pin performance.",
    ...
    },
    {
      ...
      metadata: {
        ...
        description: "Sintel is an independent CGI-animated short film by the Blender Foundation. The story follows Sintel as she searches for a baby dragon.",
        author: "Blender Foundation",
        entry: "sintel.mp4",
        tags: ["animation", "fantasy", "cgi", "open-source"],
        thumbnail: "https://upload.wikimedia.org/wikipedia/commons/f/fc/Sintel_poster_v2_small.jpg"
      }
    },
    {
      ...
      metadata: {
        ...
        description: "A sci-fi short film featuring a dystopian futuristic Amsterdam, showcasing advanced open-source VFX pipelines and tools.",
        author: "Blender Foundation",
        entry: "tears_of_steel.mp4",
        tags: ["sci-fi", "open-source", "vfx", "live-action"],
        thumbnail: "https://upload.wikimedia.org/wikipedia/commons/e/ec/Tears_of_Steel_poster.jpg"
      }
    }
  ]
};
...
async function loadCatalogData() {
  ...
  try {
    ...
    console.warn("Could not fetch mock-data.json, falling back to embedded data:", err);
  }
...
function setupEventListeners() {
  ...
  dom.btnResetMock.addEventListener('click', () => {
    if (confirm("Are you sure you want to discard your imported lists and load the demo database?")) {
      localStorage.removeItem('ipfs_video_list');
      state.videoList = FALLBACK_DEMO_DATA;
      renderUI();
      dom.settingsDialog.close();
  ...
  dom.btnCopyLink.addEventListener('click', () => {
    if (state.currentlyPlaying) {
      const directUrl = resolveIpfsOrIpnsUrl(state.currentlyPlaying.cid, state.currentlyPlaying.metadata.entry);
...
function handleUploadedFile(file) {
  dom.selectedFileInfo.textContent = `Selected: ${file.name} (${Math.round(file.size / 1024)} KB)`;
...
function validateVideoList(data) {
  ...
  for (const item of data.videos) {
    ...
    if (typeof item.metadata.title !== 'string' || !item.metadata.title.trim()) return false;
    if (typeof item.metadata.entry !== 'string' || !item.metadata.entry.trim()) return false;
...
function renderVideoGrid() {
  const filteredVideos = state.videoList.videos.filter(video => {
    ...
    const matchesSearch = !state.searchQuery || 
                          ...
                          (meta.tags && meta.tags.some(t => t.toLowerCase().includes(state.searchQuery)));
  ...
  filteredVideos.forEach(video => {
    ...
    if (thumbPath) {
      const resolvedThumb = (thumbPath.startsWith('http://') || thumbPath.startsWith('https://'))
        ? thumbPath
        : resolveIpfsOrIpnsUrl(video.cid, thumbPath);
      thumbnailHtml = `<img class="card-thumbnail" src="${resolvedThumb}" alt="${video.metadata.title} thumbnail" onerror="handleThumbError(this, '${video.metadata.title}')">`;
    } else {
    ...
    card.innerHTML = `
      ...
      <div class="card-content">
        ...
        <p class="card-description">${video.metadata.description || 'No description provided.'}</p>
    ...
    card.addEventListener('click', (e) => {
      ...
      if (e.target.closest('.btn-card-copy') || e.target.closest('.cid-badge')) {
        e.stopPropagation();
...
function showWatchView(video) {
  ...
  dom.playerDescription.textContent = video.metadata.description || 'No description provided.';
...
function renderRecommendations(currentCid) {
  ...
  const otherVideos = state.videoList.videos.filter(v => v.cid !== currentCid);
  
  if (otherVideos.length === 0) {
    dom.recommendationsContainer.innerHTML = '<p class="text-muted" style="font-size: 0.85rem; color: var(--text-muted);">No other videos available.</p>';
    return;
  }
  ...
  otherVideos.forEach(video => {
    ...
    if (thumbPath) {
      const resolvedThumb = (thumbPath.startsWith('http://') || thumbPath.startsWith('https://'))
        ? thumbPath
        : resolveIpfsOrIpnsUrl(video.cid, thumbPath);
      thumbnailHtml = `<img class="rec-thumbnail" src="${resolvedThumb}" alt="${video.metadata.title} thumbnail" onerror="handleRecThumbError(this, '${video.metadata.title}')">`;
    } else {
...
function playVideo(video) {
  ...
  const isHls = video.metadata.entry.endsWith('.m3u8') || videoUrl.includes('.m3u8');
  ...
  if (isHls) {
    if (Hls.isSupported()) {
      ...
      hlsInstance.on(Hls.Events.MANIFEST_PARSED, () => {
        videoElement.play().catch(e => console.log("Auto playback prevented:", e));
      });
        ...
        if (data.fatal) {
          switch (data.type) {
            case Hls.ErrorTypes.NETWORK_ERROR:
              showPlayerError("Network error: Could not fetch video segments from IPFS gateway. Please check gateway connection or verify CID exists.");
              break;
    ...
    // Check for native Safari HLS support
      ...
      showPlayerError("Your browser does not support HLS streaming (HTTP Live Streaming).");
    }
  } else {
    ...
    videoElement.play().catch(e => console.log("Auto playback prevented:", e));
    videoElement.addEventListener('error', () => {
      showPlayerError("Failed to play video file. Please check if file exists on gateway.");
    });
  }
}
...
function resolveIpfsOrIpnsUrl(cidOrKey, subpath = '') {
  ...
  if (isIpns) {
    if (baseGateway.endsWith('/ipfs/')) {
      ...
      baseGateway = baseGateway.endsWith('/') ? `${baseGateway}ipns/` : `${baseGateway}/ipns/`;
    }
  } else {
    // Standard IPFS
    if (!baseGateway.includes('/ipfs/') && !baseGateway.includes('/ipns/')) {
      baseGateway = baseGateway.endsWith('/') ? `${baseGateway}ipfs/` : `${baseGateway}/ipfs/`;
    }
  }

  let finalUrl = `${baseGateway}${cleanCid}`;
  if (subpath) {
    const cleanSubpath = subpath.trim().startsWith('/') ? subpath.trim().substring(1) : subpath.trim();
    finalUrl = finalUrl.endsWith('/') ? `${finalUrl}${cleanSubpath}` : `${finalUrl}/${cleanSubpath}`;
  }
